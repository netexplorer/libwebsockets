(function() {
	...
	function humanize(n)
	{
		...
		if (n < 1024 * 1024)
			return (n / 1024).toFixed(3) + "KiB";
		
		if (n < 1024 * 1024 * 1024)
			return (n / (1024 * 1024)).toFixed(3) + "MiB";
		
		return (n / (1024 * 1024 * 1024)).toFixed(3) + "GiB";
	}
	...
	function do_upload(file) {
		  ...
		  fetch("upload/" + encodeURI(file.name), {
		    method: 'POST',
		    body: formData
		  })
		  .then((e) => { /* this just means we got a response code */			  
			  ...
			  for (n = 0; n < t.rows.length; n++)
				  if (ul === encodeURI(t.rows[n].cells[2].textContent)) {
					  if (e.ok === true) {
						  t.deleteRow(n);
					  } else {
						  t.rows[n].cells[0].textContent =
							  "Failed " + san(e.status.toString());
					      t.rows[n].cells[0].classList.add("err");
					  }
					  break;
				  }
		  })
			...
			ws.onmessage = function got_packet(msg) {
				...
				for (n = 0; n < j.files.length; n++) {
					...
					if (j.files[n].yours === 1)
						s += "<img id=\"d" + n + "\" class=\"delbtn\" file=\"" +
							encodeURI(san(j.files[n].name)) + "\">";
					else
						s += " ";

					s += "</td><td class=\"ogn\"><a href=\"get/" +
						 encodeURI(san(j.files[n].name)) + "\" download>" +
